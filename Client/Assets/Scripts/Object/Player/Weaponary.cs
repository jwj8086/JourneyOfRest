...
public class Weaponary : MonoBehaviour {
    ...
    private WaitForSeconds _firingIntervalWait;
    ...
    private void Awake() {
        ...
        if(null == _firingIntervalWait)
            _firingIntervalWait = new WaitForSeconds(_firingInterval);
    ...
    public void PullTrigger(Vector2 dir, int firingCount) {
    ...
    private IEnumerator CoStartFireBullets(Vector2 dir, int firingCount) {
        for(int i = 0; i < firingCount; i++) {
            FireBulletOnce(dir);
            yield return _firingIntervalWait;
        }
